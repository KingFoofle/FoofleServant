module.exports = async (client, message) => {
	const { author: user, content, channel } = message,
		...
		member = getMemberFromUserId(user.id);
	...
	if (content.startsWith(PREFIX)) {
		...
		try {
			...
			if (command) {
				...
				let result;
				if (command.canBeUsed) result = command.canBeUsedBy(client, member);

				// Check if the member can use the command
				if (result && result.reason) {channel.reply(`You cannot use this command!\nReason: ${result.reason}`);}
